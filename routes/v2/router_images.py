...
async def files_m(empresa: str = Form(...), imagenes: List[UploadFile] = File(...)):
    resultado = {}
    resultado = []
    ruta_archivo_temporal = f"{os.getcwd()}\\server\\images\\{empresa}_temp\\"
    ruta_archivo = f"{os.getcwd()}\\server\\images\\{empresa}\\"
    ...
    for imagen in imagenes:
        ...
        if img.count('image') == 1:
            with open(f"{ruta_archivo_temporal}{imagen.filename}", "wb") as myfile:
...
async def s_images(empresa: str, imagen: str):
    busqueda = os.path.isfile(f"{os.getcwd()}\\server\\images\\{empresa}\\{imagen}")
    file_img = f"{os.getcwd()}\\server\\images\\{empresa}\\{imagen}"
    if busqueda == True:
